...
function truncatePrompt(prompt: string, maxLen: number = PROMPT_MAX_LEN): string {
  ...
  return prompt.slice(0, breakAt).trimEnd() + '...';
}
...
export function renderScoreboard(tasks: TaskState[]): string {
  ...
  for (const task of sorted) {
    ...
    const labelTag = task.labels.length > 0 ? ` [${task.labels.join(',')}]` : '';
    lines.push(`${badge} ${task.id}${labelTag} -- "${prompt}" (${elapsed})`);
  }

  // Footer with quick-reference instructions
  lines.push('');
  lines.push('> Details: read `task:///<id>` · Events: read `task:///<id>/events` · Poll: read `task:///all` every ~30s');
  ...
  if (tasksWithQuestions.length > 0) {
    ...
    for (const task of tasksWithQuestions) {
      ...
      if (pq.type === 'user_input') {
        ...
        for (const q of pq.questions) {
          answersExample[q.id] = q.options?.length ? '1' : 'your answer';
        }
        lines.push('');
        lines.push(`Answer: \`respond-task ${JSON.stringify({ task_id: task.id, type: 'user_input', answers: answersExample })}\``);
      } else {
