...
import type { TaskFileWriter } from '../services/task-file-writer.js';
...
import { formatTimelineLine } from '../timeline-writer.js';
        ...
        return {
          ...
          ...(asArray(q.options)
            ? { options: (q.options as unknown[]).map((opt) => {
                ...
                return String(o?.label ?? o?.value ?? o?.description ?? JSON.stringify(opt));
              }) }
            : {}),
...
export function attachPauseFlow(
  ...
  handle: TaskHandle,
  threadId: string,
  fileWriter?: TaskFileWriter | undefined,
): () => void {
  const onServerRequest = (pending: PendingServerRequest) => {
    ...
    // Log EVERY server-request to events.jsonl — server-requests are
    // invisible in notifications, so without this the question flow
    // and approval flow have no trace in the event log.
    if (fileWriter) {
      fileWriter.appendEvent(handle.taskId, {
        method: `_server_request:${pending.method}`,
        requestId: pending.id,
        params: pending.params,
      }).catch(() => {});
      // Timeline: show the server-request as an event
      const tlResult = formatTimelineLine({
        t: new Date().toISOString(),
        method: pending.method,
        params: pending.params,
      });
      if (tlResult.line) {
        fileWriter.appendTimeline(handle.taskId, tlResult.line).catch(() => {});
      }
    }
    ...
    const params = asObject(pending.params);

    // -----------------------------------------------------------------
    // Auto-approve: command approvals, file approvals, elicitations,
    // and permission requests. Respond instantly so the agent never
    // stalls waiting for orchestrator input on these.
    // -----------------------------------------------------------------

    // Command approval → accept immediately
    if (pending.method === 'item/commandExecution/requestApproval' ||
        pending.method === 'execCommandApproval') {
      const cmd = asString(params?.command) ?? 'unknown';
      handle.writeOutput(`[auto-approve] cmd: ${cmd}`);
      client.respondToServerRequest(pending.id, { decision: 'accept' }).catch(() => {});
      return;
    }

    // File change approval → accept immediately
    if (pending.method === 'item/fileChange/requestApproval' ||
        pending.method === 'applyPatchApproval') {
      handle.writeOutput('[auto-approve] file change');
      client.respondToServerRequest(pending.id, { decision: 'accept' }).catch(() => {});
      return;
    }

    // MCP elicitation → accept immediately
    if (pending.method === 'mcpServer/elicitation/request') {
      handle.writeOutput('[auto-approve] elicitation');
      client.respondToServerRequest(pending.id, {
        action: 'accept',
        content: {},
      }).catch(() => {});
      return;
    ...
    // Permission request → grant all requested permissions for the session
    if (pending.method === 'item/permissions/requestApproval') {
      const perms = asObject(params?.permissions);
      handle.writeOutput('[auto-approve] permissions');
      client.respondToServerRequest(pending.id, {
        scope: 'session',
        permissions: perms ?? {},
      }).catch(() => {});
    ...
    // -----------------------------------------------------------------
    // Auto-answer: requestUserInput — the Codex process has a ~4s
    // timeout for responses. The orchestrator can't respond that fast
    // (wait-task polling + LLM thinking + respond-task call = 5-30s).
    // So we auto-select the first (recommended) option for each
    // question and log the choices. If the orchestrator disagrees,
    // they can steer via message-task.
    // -----------------------------------------------------------------
    if (pending.method === 'tool/requestUserInput' ||
        pending.method === 'item/tool/requestUserInput') {
      const rawQuestions = asArray(params?.questions) ?? [];
      // ToolRequestUserInputResponse = { answers: { [qId]: ToolRequestUserInputAnswer } }
      // ToolRequestUserInputAnswer   = { answers: string[] }
      const autoAnswers: Record<string, { answers: string[] }> = {};
      ...
      for (const raw of rawQuestions) {
        ...
        if (options && options.length > 0) {
          // Pick the first option (recommended per QUESTION POLICY)
          const firstOpt = options[0];
          if (typeof firstOpt === 'string') {
            chosenLabel = firstOpt;
          } else {
            const o = asObject(firstOpt);
            chosenLabel = asString(o?.label) ?? asString(o?.value) ?? String(firstOpt);
          }
        } else {
          chosenLabel = 'yes';
        }

        // Each answer must be { answers: string[] } per ToolRequestUserInputAnswer
        autoAnswers[id] = { answers: [chosenLabel] };
        logParts.push(`${text} → "${chosenLabel}"`);
      }

      client.respondToServerRequest(pending.id, { answers: autoAnswers }).catch(() => {});
      handle.writeOutput(`[auto-answer] ${logParts.join(' | ')}`);
      // Log to events.jsonl and timeline
      if (fileWriter) {
        const answerSummary = logParts.join(' | ');
        fileWriter.appendEvent(handle.taskId, {
          method: '_auto_answer',
          answers: autoAnswers,
        ...
        const line = `${ts} AUTO    ${answerSummary.length > 450 ? answerSummary.slice(0, 447) + '...' : answerSummary}`;
        fileWriter.appendTimeline(handle.taskId, line).catch(() => {});
      }
      return;
    }

    // -----------------------------------------------------------------
    // Queue: dynamic_tool still needs real answers from the
    // orchestrator — this is the only type that pauses the task.
    // -----------------------------------------------------------------
