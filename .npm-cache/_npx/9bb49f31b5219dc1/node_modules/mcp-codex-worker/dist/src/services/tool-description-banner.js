...
function truncateBanner(text) {
    if (text.length <= BANNER_MAX_CHARS)
        return text;
    return text.slice(0, BANNER_MAX_CHARS - 3) + '...';
}
/**
 ...
 */
export function buildStatusBanner(allTasks) {
    const now = Date.now();
    const running = [];
    const needsAnswer = [];
    const recentlyDone = [];
    ...
    const summaryParts = [];
    if (running.length > 0)
        summaryParts.push(`${running.length} running`);
    if (needsAnswer.length > 0)
        summaryParts.push(`${needsAnswer.length} needs answer`);
    if (recentlyDone.length > 0)
        summaryParts.push(`${recentlyDone.length} recently finished`);
    ...
    return truncateBanner(parts.join('\n'));
}
/**
 ...
 */
export function buildRespondBanner(allTasks) {
    const tasksWithQuestions = allTasks
...
//# sourceMappingURL=tool-description-banner.js.map