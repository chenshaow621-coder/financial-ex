...
const OPERATION_EVICTION_MS = 5 * 60 * 1000;
const MAX_OPERATIONS = 2_000;
const MAX_SERVER_REQUESTS = 2_000;
export class AppServerClient extends EventEmitter {
    ...
    async start() {
        ...
        // If an API key is available in the environment, log in with it to
        // override any stale OAuth tokens in auth.json. This prevents the
        // "refresh token already used" crash that kills the app-server
        // during long tasks when the cached ChatGPT OAuth token expires.
        const apiKey = this.options.env?.CODEX_LB_API_KEY
            ?? this.options.env?.OPENAI_API_KEY;
        if (apiKey) {
            await this.request('account/login/start', {
                type: 'apiKey',
                apiKey,
            }).catch(() => {
                // Non-fatal — the server may not support this auth mode,
                // or may already be authenticated.
            });
        }
    }
    async stop() {
    ...
    listOperationsForThread(threadId) {
    ...
    updateOperationsFromNotification(notification) {
        ...
        if (notification.method === 'error') {
            ...
            for (const operation of this.operations.values()) {
                ...
                if (notificationTurnId && operation.turnId && operation.turnId !== notificationTurnId) {
                    continue;
                }
                operation.status = 'failed';
                operation.completedAt = new Date().toISOString();
     ...
     */
    extractErrorNotificationMessage(params) {
    ...
    evictStaleEntries() {
        ...
        for (const [key, op] of this.operations) {
            if ((op.status === 'completed' || op.status === 'failed') && op.completedAt) {
                const age = now - new Date(op.completedAt).getTime();
                if (age > OPERATION_EVICTION_MS) {
                    this.operations.delete(key);
                    this.operationsByRequestId.delete(requestIdKey(op.requestId));
                }
            }
        }
        if (this.operations.size > MAX_OPERATIONS) {
            const sorted = [...this.operations.entries()]
                .filter(([, op]) => op.status !== 'running')
                .sort((a, b) => (a[1].completedAt ?? '').localeCompare(b[1].completedAt ?? ''));
            for (const [key, op] of sorted) {
                this.operations.delete(key);
                this.operationsByRequestId.delete(requestIdKey(op.requestId));
                if (this.operations.size <= MAX_OPERATIONS) {
                    break;
        ...
        for (const [key, req] of this.serverRequests) {
            if (req.status === 'resolved' && req.resolvedAt) {
                const age = now - new Date(req.resolvedAt).getTime();
                if (age > OPERATION_EVICTION_MS) {
                    this.serverRequests.delete(key);
                }
            }
        }
        if (this.serverRequests.size > MAX_SERVER_REQUESTS) {
            const resolved = [...this.serverRequests.entries()]
                .filter(([, r]) => r.status === 'resolved')
                .sort((a, b) => (a[1].resolvedAt ?? '').localeCompare(b[1].resolvedAt ?? ''));
