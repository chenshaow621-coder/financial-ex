...
// Main entry point
// ---------------------------------------------------------------------------
export function formatTimelineLine(event, config = DEFAULT_TIMELINE_CONFIG, lastTokenCount) {
    ...
    switch (event.method) {
        case 'thread/status/changed': {
            ...
            if (status === 'active')
                return line(ts, 'STARTED', '', config);
            // Skip 'idle' — it fires right before turn/completed and adds no value.
            // The DONE line already communicates task completion.
            if (status === 'waitingOnUserInput')
                return line(ts, 'ASK', 'waiting for orchestrator', config);
            return SKIP;
        }
        case 'turn/started': {
        ...
        case 'item/started': {
            // Only reasoning start — gives "agent is alive" signal during silent gaps
            const startedItem = asObj(p?.item);
            if (asStr(startedItem?.type) === 'reasoning') {
                return line(ts, 'THINK', '(reasoning…)', config);
            }
            return SKIP;
        }
        case 'item/completed':
        ...
        case 'thread/tokenUsage/updated': {
            ...
            const window = typeof asObj(p?.tokenUsage)?.modelContextWindow === 'number'
                ? asObj(p?.tokenUsage).modelContextWindow
                : undefined;
            if (!usage || !window)
                return SKIP;
            const total = typeof usage.totalTokens === 'number' ? usage.totalTokens : 0;
            if (total === (lastTokenCount ?? -1))
                return SKIP; // deduplicate
...
// ---------------------------------------------------------------------------
function formatItemCompleted(ts, item, config) {
    if (!item)
        return SKIP;
    ...
    switch (itemType) {
        case 'commandExecution': {
            ...
            const durMs = typeof item.durationMs === 'number' ? item.durationMs : undefined;
            const dur = durMs === undefined ? '?' : durMs <= 10 ? '<1ms' : `${(durMs / 1000).toFixed(1)}s`;
        ...
        case 'reasoning': {
            ...
            const first = typeof summaries[0] === 'string'
                ? summaries[0]
                : asStr(summaries[0]?.text) ?? '';
            // Extract only the bold heading (e.g. "**Planning execution steps**")
            // before the first double-newline or the first sentence that starts
            // with "I need", "I should", etc. (internal reasoning noise).
            const boldMatch = first.match(/\*\*(.+?)\*\*/);
            const heading = boldMatch ? boldMatch[1] : first.split(/\n\n/)[0];
            const cleaned = heading.replace(/\*\*/g, '').replace(/\n/g, ' ').trim();
            if (!cleaned)
                return SKIP;
...
// Plan formatter
// ---------------------------------------------------------------------------
function formatPlan(steps, config) {
    if (steps.length === 0)
        return '(empty plan)';
    const icon = (s) => {
        switch (s) {
...
// Helpers
// ---------------------------------------------------------------------------
const SKIP = { line: null };
function pad(tag) {
    return tag.padEnd(7);
}
function line(ts, tag, detail, config) {
    return { line: capLine(`${ts} ${pad(tag)} ${detail}`.trimEnd(), config) };
}
function capLine(l, config) {
    if (l.length <= config.maxLineLength)
        return l;
...
export function cleanCommand(raw) {
    const match = raw.match(/^\/bin\/(?:z?sh|bash)\s+-[a-z]*c\s+['"]?([\s\S]+?)['"]?$/);
    let cmd = match ? match[1] : raw;
    // Strip all rtk prefixes — not just the leading one. Chained commands
    // like "rtk find ... && rtk wc -l ..." need every occurrence removed.
    cmd = cmd.replace(/\brtk /g, '');
    return cmd;
}
...
//# sourceMappingURL=timeline-writer.js.map